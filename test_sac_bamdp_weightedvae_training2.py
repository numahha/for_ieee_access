...
vi.load("policy_optimization_iter2")
...
agent.load_checkpoint(ckpt_path="checkpoints/sac_checkpoint_custom_"+env_str+"_bamdp_weightedvae_", evaluate=False)
#Tesnorboard
...
for i_episode in itertools.count(1):
    ...
    if i_episode % 10 == 0 and args.eval is True:
        ...
        avg_reward /= episodes
        agent.save_checkpoint(env_name="custom_"+env_str+"_bamdp_weightedvae2")
        ...
        if cfg_env == "pendulum":
            plt.ylim([-300,0])
        plt.legend()
        plt.savefig("fig_policy_optimization_curve_weightedvae2.png")
        plt.close()
        ...
        if cfg_env == "pendulum":
            ...
            plt.savefig("fig_tmpw2_angle_pendulum.png")
            plt.close()

        if cfg_env == "cartpole":
            plt.plot(state_history[:,0], state_history[:,1])
            plt.plot(state_history[0,0], state_history[0,1],"o")
            plt.savefig("fig_tmpw2_position_cartpole.png")
            plt.close()
            plt.plot(state_history[:,2], state_history[:,3])
            plt.plot(state_history[0,2], state_history[0,3],"o")
            plt.savefig("fig_tmpw2_angle_cartpole.png")
            plt.close()
